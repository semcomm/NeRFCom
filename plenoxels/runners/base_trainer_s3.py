...
from plenoxels.models.lowrank_model_s3 import LowrankModel
...
class BaseTrainer(abc.ABC):
    ...
    def freeze_ntc(self):
        ...
        for name, param in self.model.named_parameters():
            if "ntc" in name and param.requires_grad:
                param.requires_grad = False
                # print(f"❄️  NTC model {name}")
            else:
    ...
    def hot_nerf(self):
        print(f"🔥  NeRF model")
        for name, param in self.model.named_parameters():
            if "ntc" not in name and not param.requires_grad and "aabb" not in name:
                param.requires_grad = True
                # print(f"🔥  NeRF model {name}")
            else:
                print(f"Pass params {name}")
    ...
    def train_step(self, data, **kwargs) -> bool:
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16):
            ...
            # Regularization
            ntc_loss_weight = 1e-4
            bpp_weight = 1e-3
            loss = recon_loss + ntc_loss_weight * ntc_mse_loss + bpp_weight * (ntc_bpp_y + ntc_bpp_z)
            # loss = recon_loss
        ...
        if self.global_step % self.calc_metrics_every == 0:
            with torch.no_grad():
                ...
                #         "cur_nerf_lr": cur_nerf_lr, "cur_ntc_lr": cur_ntc_lr, "scale": scale,
    ...
    def post_step(self, progress_bar):
        ...
        if self.global_step % self.calc_metrics_every == 0:
            progress_bar.set_postfix_str(losses_to_postfix(self.loss_info, lr=self.lr), refresh=False)
