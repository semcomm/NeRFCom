...
from plenoxels.models.lowrank_model_s2 import LowrankModel
...
os.environ["HTTP_PROXY"] = "http://10.106.130.4:3128"
os.environ["HTTPS_PROXY"] = "https://10.106.130.4:3128"
...
class BaseTrainer(abc.ABC):
    def __init__(self,
        ...
        self.optimizer, self.optim_ntc, self.optim_aux = self.init_optim(**self.extra_args)
        self.scheduler, self.sche_ntc, self.sche_aux = self.init_lr_scheduler(**self.extra_args)
        ...
        time_now = f'{datetime.datetime.now().strftime("%m%d")}'
        exp_name = f"s2_{time_now}"
        # self.wandb_board = wandb.init(
        #     project="KPlanes_NTC_static",
        #     name= time_now if exp_name is None else exp_name,
        #     config={
        #         "learning_rate": None,
        #         "architecture": "Hyperprior",
        #         "dataset": "static",
        #         "epochs": 60000,
        #     }
        # )
    ...
    def hot_ntc(self):
        # field.ntc_cnn_encoder.0.weight torch.Size([48, 96, 3, 3])
        # field.ntc_cnn_encoder.0.bias torch.Size([48])
        print(f"🔥  NTC model")
        for name, param in self.model.named_parameters():
            if "ntc" in name and not param.requires_grad:
                param.requires_grad = True
                # print(f"🔥  NTC params: {name}")
            else:
                print(f"Pass params {name}")

    def freeze_nerf(self):
        print(f"❄️  NeRF model")
        for name, param in self.model.named_parameters():
            if "ntc" not in name and param.requires_grad:
                param.requires_grad = False
                # print(f"❄️  NeRF params {name}")
            else:
                print(f"Pass params {name}")
    ...
    def train_step(self, data, **kwargs) -> bool:
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16):
            ...
            ntc_res = fwd_out['ntc_res']
            ntc_mse_loss = ntc_res['ntc_mse_loss']
            ntc_bpp_y = ntc_res['ntc_bpp_y']
            ntc_bpp_z = ntc_res['ntc_bpp_z']

            # Reconstruction loss
            recon_loss = self.criterion(fwd_out['rgb'], data['imgs'])
            # Regularization
            ntc_loss_weight = 1e-3
            bpp_weight = 1e-2
            # print(f"ntc_mse_loss={ntc_mse_loss.device}")
            loss = recon_loss + ntc_loss_weight * ntc_mse_loss.to('cpu') + bpp_weight * (ntc_bpp_y + ntc_bpp_z)
            for r in self.regularizers:
                reg_loss = r.regularize(self.model, model_out=fwd_out)
                loss = loss + reg_loss
            self.timer.check("regularization-forward")

        cur_nerf_lr = self.optimizer.param_groups[0]['lr']
        cur_ntc_lr = self.optim_ntc.param_groups[0]['lr']
        cur_aux_lr = self.optim_aux.param_groups[0]['lr']
        ...
        # self.optim_ntc.zero_grad(set_to_none=True)
        # loss.backward()
        # self.optim_ntc.step()
        ...
        self.gscaler.scale(loss).backward()
        self.timer.check("backward")
        self.gscaler.step(self.optim_ntc)
        scale = self.gscaler.get_scale()
        self.gscaler.update()
        self.timer.check("scaler-step")
        ...
        if self.global_step % self.calc_metrics_every == 0:
            with torch.no_grad():
                ...
                psnr = -10 * math.log10(recon_loss_val)
                self.loss_info[f"psnr"].update(psnr)
                for r in self.regularizers:
                    r.report(self.loss_info)

                # self.wandb_board.log(
                #     {
                #         "mse": recon_loss_val, "psnr": psnr,
                #         "cur_nerf_lr": cur_nerf_lr, "cur_ntc_lr": cur_ntc_lr,
                #         "scale": scale,
                #         "ntc_mse_loss": ntc_mse_loss.item(),
                #         "ntc_bpp_y": ntc_bpp_y.item(),
                #         "ntc_bpp_z": ntc_bpp_z.item(),
                #     }
                # )
    ...
    def train(self):
        ...
        try:
            ...
            while self.global_step < self.num_steps:
                ...
                #     self.scheduler.step()
                if step_successful and self.sche_ntc is not None:
                    self.sche_ntc.step()
                for r in self.regularizers:
    ...
    def evaluate_metrics(self,
        ...
        preds_rgb = (
            preds["rgb"].reshape(img_h, img_w, 3).cpu().clamp(0, 1)
        )
        ...
        if gt is not None:
            ...
            if gt.shape[-1] == 4:
                gt = gt[..., :3] * gt[..., 3:] + (1.0 - gt[..., 3:])
            metrics_res = self.calc_metrics(preds_rgb, gt)
            summary.update(metrics_res)
        ...
        psnr_value = summary['psnr']
        ssim_value = summary['ssim']
        ms_ssim_value = summary['ms-ssim']
        alex_lpips_value = summary['alex_lpips']
        vgg_lpips_value = summary['vgg_lpips']

        if save_outputs:
            out_name = f"step{self.global_step}-{img_idx}"
            if name is not None and name != "":
                out_name += "-" + name
            res_path = os.path.join(self.log_dir,
                                    f"{out_name}_"
                                    f"bpp_y_{bpp_y:.8f}_"
                                    f"psnr_{psnr_value:.4f}_"
                                    f"ssim_{ssim_value:.4f}_"
                                    f"ms_ssim_{ms_ssim_value:.4f}_"
                                    f"alex_lpips_{alex_lpips_value:.4f}_"
                                    f"vgg_lpips_{vgg_lpips_value:.4f}.png")
            print(f"res_path={res_path}")
            write_png(res_path, out_img_np)
            if out_depth is not None:
    ...
    def get_save_dict(self):
        return {
            ...
            "optim_ntc": self.optim_ntc.state_dict(),
            "sche_ntc": self.sche_ntc.state_dict(),
    ...
    def load_weights(self, checkpoint_data):
        result_dict = {}
        for key, weight in checkpoint_data.items():
            result_key = key
        ...
        self.is_load_weight = True
        del result_dict

    def load_model(self, checkpoint_data, training_needed: bool = True):
        self.load_weights(checkpoint_data["model"])
        ...
        if training_needed:
            try:
                ...
                print(f"❌ Ignored while loading optimizer state: {e}")
        ...
        if training_needed and self.scheduler is not None:
            try:
                ...
                print(f"❌ Ignored while loading scheduler state: {e}")



        self.global_step = checkpoint_data["global_step"] - 30000
    ...
    def init_lr_scheduler(self, **kwargs) -> Optional[torch.optim.lr_scheduler._LRScheduler]:
        ...
        log.info(f"Initializing LR Scheduler of type {scheduler_type} with " # warmup_cosine
                 f"{max_steps} maximum steps.")
        if scheduler_type == "cosine":
            lr_sched = torch.optim.lr_scheduler.CosineAnnealingLR(
                self.optimizer, T_max=max_steps, eta_min=eta_min)
        elif scheduler_type == "warmup_cosine":
            lr_sched = get_cosine_schedule_with_warmup(
                self.optimizer, num_warmup_steps=512, num_training_steps=max_steps)

            lr_ntc_sched = get_cosine_schedule_with_warmup(
                self.optim_ntc, num_warmup_steps=512, num_training_steps=max_steps)

            lr_aux_sched = get_cosine_schedule_with_warmup(
                self.optim_aux, num_warmup_steps=512, num_training_steps=max_steps)
        ...
        return lr_sched, lr_ntc_sched, lr_aux_sched

    def init_optim(self, **kwargs) -> torch.optim.Optimizer:
        optim_type = kwargs['optim_type']
        if optim_type == 'adam':
            params = self.model.get_params(kwargs['lr'])
            optim = torch.optim.Adam(params=params[:3], eps=1e-15)
            optim_ntc = torch.optim.Adam(params=[params[3]])
            optim_aux = torch.optim.Adam(params=[params[4]])
        else:
            raise NotImplementedError()
        return optim, optim_ntc, optim_aux
