...
# from .base_trainer import BaseTrainer, init_dloader_random, initialize_model # train nerf, without NTSCC
# from .base_trainer_s2 import BaseTrainer, init_dloader_random, initialize_model # freeze nerf, train NTSCC
from .base_trainer_s3 import BaseTrainer, init_dloader_random, initialize_model # train nerf, freeze NTSCC
...
class StaticTrainer(BaseTrainer):
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16), torch.no_grad():
            ...
            for b in range(math.ceil(rays_o.shape[0] / batch_size)):
                ...
                outputs = self.model(rays_o_b, rays_d_b, near_far=near_far,
                                     bg_color=bg_color)
                bpp_y_value = outputs['ntc_res']['ntc_bpp_y'].item()
                ntc_mse_loss = outputs['ntc_res']['ntc_mse_loss'].item()

                print(f"bpp_y_value: {bpp_y_value}, ntc_mse_loss={ntc_mse_loss}")
                bpp_y_lst.append(bpp_y_value)

                for k, v in outputs.items():
                    if (k in channels or "depth" in k):
                        preds[k].append(v.cpu())

        return {k: torch.cat(v, 0) for k, v in preds.items()}, np.mean(bpp_y_lst)
        # return preds
    ...
    def validate(self):
        ...
        for img_idx, data in enumerate(dataset):
            ts_render, bpp_y = self.eval_step(data)
            for k, v in ts_render.items():
                print(k, v.shape)
            out_metrics, _, _ = self.evaluate_metrics(
                data["imgs"], ts_render, bpp_y, dset=dataset, img_idx=img_idx,
                name=None, save_outputs=self.save_outputs)
            for k, v in out_metrics.items():
                per_scene_metrics[k].append(v)
            psnr_value = out_metrics['psnr']
            pb.set_postfix_str(f"PSNR={psnr_value:.2f}", refresh=False)
            pb.update(1)
        pb.close()
        val_metrics = [
