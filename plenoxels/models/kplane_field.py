...
from plenoxels.models.NTC_Hyperprior import NTC_Hyperprior
...
class KPlaneField(nn.Module):
    ...
    def get_density(self, pts: torch.Tensor, timestamps: Optional[torch.Tensor] = None):
        ...
        if self.concat_features:
            ...
            grids_tsr = torch.cat(grid_param_lst, dim=0) / 16
            mse_loss, bpp_y, bpp_z, grids_hat = self.ntc_model(grids_tsr)
            ...
            #       f"grids_hat [{torch.min(grids_hat)}-{torch.max(grids_hat)}]")
            grids_hat = grids_hat * 16
