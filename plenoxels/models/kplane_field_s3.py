...
from plenoxels.models.NTC_Hyperprior import NTC_Hyperprior, NTSCC_Hyperprior
from plenoxels.models.NTC_utils.config import config as NTC_config
...
class KPlaneField(nn.Module):
    def __init__(
        ...
        self.ntc_model = NTC_Hyperprior(NTC_config)
    ...
    def get_density(self, pts: torch.Tensor, timestamps: Optional[torch.Tensor] = None):
        ...
        # =========== 🔥🔥🔥🔥 ===========
        if self.concat_features:
            grid_param_lst = [grid for grids in self.grids for grid in grids]
            # for i, grid in enumerate(grid_param_lst):
            #     print(f"grid_param_lst[{i}]: {grid.size()}")
            grids_tsr = torch.cat(grid_param_lst, dim=0)
            print(f"\ninit grids_tsr [{torch.min(grids_tsr):.4f}-{torch.max(grids_tsr):.4f}] ")

            # grids_tsr = grids_tsr / 20 + 0.5
            grids_tsr = grids_tsr / 20
            print(f"grids_tsr [{torch.min(grids_tsr):.4f}-{torch.max(grids_tsr):.4f}] ")


            mse_loss, bpp_y, bpp_z, x_hat_ntc = self.ntc_model(grids_tsr)
            # mse_loss_ntc, bpp_y, bpp_z, mse_loss_ntscc, cbr_y, x_hat_ntc, x_hat_ntscc = self.ntc_model(grids_tsr)
            print(f"x_hat_ntc [{torch.min(x_hat_ntc)}-{torch.max(x_hat_ntc)}]")
            # print(f"x_hat_ntscc [{torch.min(x_hat_ntscc)}-{torch.max(x_hat_ntscc)}]")

            # grids_hat = (x_hat_ntscc - 0.5 * torch.ones(grids_tsr.shape).to(grids_tsr.device)) * 20
            # grids_hat = (x_hat_ntscc - 0.5) * 20
            grids_hat = x_hat_ntc * 20


            # print(f"grids_tsr [{torch.min(grids_tsr)}-{torch.max(grids_tsr)}] ")
            print(f"grids_hat [{torch.min(grids_hat)}-{torch.max(grids_hat)}]")
            # mse_loss, bpp_y, bpp_z, grids_hat = torch.zeros(1), torch.zeros(1), torch.zeros(1), torch.zeros(1)
            ntc_res = {
                'ntc_mse_loss': mse_loss,
                'ntc_bpp_y': bpp_y,
                'ntc_bpp_z': bpp_z,
            }

            grids_hat = [torch.chunk(grids_hat, chunks=grids_hat.size(0), dim=0)]
            # print(f"grids_hat_list: {len(grids_hat_list)} {len(grids_hat_list[0])}")
            # for i, grids_hat_tmp in enumerate(grids_hat_list):
            #     for j, grid_hat_tmp in enumerate(grids_hat_tmp):
            #         print(f"{i} {j} {grid_hat_tmp.size()}")
        else:
        ...
        return density, features, ntc_res
    ...
    def forward(self,
        ...
        if self.use_appearance_embedding:
            ...
            timestamps = None
        density, features, ntc_res = self.get_density(pts, timestamps)
        n_rays, n_samples = pts.shape[:2]
        ...
        return {"rgb": rgb, "density": density, "ntc_res": ntc_res}
    ...
    def get_params(self):
        ...
        ntc_params = {k: v for k, v in self.named_parameters() if
                      (k.startswith("ntc") or k.startswith("entropy")) and not k.endswith(".quantiles")
                      and k not in field_params.keys()}
        aux_params = {k: v for k, v in self.named_parameters() if
                      (k.startswith("ntc") or k.startswith("entropy")) and k.endswith(".quantiles")
                      and k not in field_params.keys()}

        other_params = {k: v for k, v in self.named_parameters() if (
            k not in nn_params.keys() and k not in field_params.keys() and k not in ntc_params.keys() and k not in aux_params.keys()
        )}
        return {
            "nn": list(nn_params.values()),
            "field": list(field_params.values()),
            "other": list(other_params.values()),
            "ntc": list(ntc_params.values()),
            "aux": list(aux_params.values()),
        }
