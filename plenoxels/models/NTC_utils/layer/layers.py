...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = x
        x = self.norm1(x)
        # print(f"H, W, C, self.window_size={H, W, C, self.window_size} x.shape={x.shape}")
...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=256, patch_size=2, in_chans=3, embed_dim=128, norm_layer=None):
        ...
        # print("img_size", img_size, "patch_size", patch_size, "patches_resolution", patches_resolution)
    ...
    def forward(self, x):
        # print(f"PatchEmbed forward:")
        # print(f"x [{torch.min(x):.4f}-{torch.max(x):.4f}] ")
        x = self.proj(x)
        # print(f"x [{torch.min(x):.4f}-{torch.max(x):.4f}] {x.shape}")
