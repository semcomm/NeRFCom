...
from plenoxels.models.NTC_utils.layer.layers import SwinTransformerBlock, PatchEmbed, PatchMerging
import torch
...
class AnalysisTransform(nn.Module):
    def __init__(self, img_size=(256, 256),
        ...
        # ⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️ in_chans需要修改
        self.patch_embed = PatchEmbed(img_size=img_size, patch_size=2, in_chans=32, embed_dim=embed_dims[0], norm_layer=nn.LayerNorm)
        ...
        for i_layer in range(self.num_layers):
            ...
            # print(f"i_layer={i_layer}, input_resolution={input_resolution}")
    ...
    def forward(self, x):
        # print(f"ga forward")
        # print(f"x [{torch.min(x):.4f}-{torch.max(x):.4f}] ")
        ...
        for i_layer, layer in enumerate(self.layers):
            x = layer(x)
            # print(f"{i_layer} x [{torch.min(x):.4f}-{torch.max(x):.4f}] ")
...
def build_model(config):
    input_image = torch.ones([1, 96, 4096, 64]).to(config.device)
